<template>
  <div class="fixed inset-0 z-[100] flex items-center justify-center p-4">
    ...
    <div class="relative w-full max-w-lg rounded-xl bg-white shadow-2xl animate-fade-up overflow-hidden border border-slate-100 dark:bg-[#1a1d26] dark:border-white/10">
      ...
      <div class="relative p-8">
        <div class="flex items-start justify-between gap-4">
          ...
          <button
            type="button"
            class="rounded-full p-2 text-slate-400 hover:bg-slate-100 hover:text-slate-600 transition-colors focus:outline-none focus:ring-2 focus:ring-trc-light/50 dark:hover:bg-white/10 dark:hover:text-slate-300"
            @click="close()"
            aria-label="Close"
          >
        ...
        <form @submit.prevent="submit" class="mt-8 space-y-6">
          <div class="space-y-4">
            ...
            <div class="space-y-1.5">
              <label class="block text-xs font-black uppercase tracking-widest text-slate-500 dark:text-slate-400">
                Description <span class="text-slate-400 font-semibold normal-case tracking-normal dark:text-slate-500">(Optional)</span>
              </label>
              <textarea
                v-model="form.description"
                rows="3"
                class="w-full rounded-xl border border-slate-200 bg-white px-4 py-3 text-sm font-semibold text-slate-800 outline-none transition-all placeholder:text-slate-400 focus:border-trc-light focus:ring-4 focus:ring-trc-light/10 resize-none dark:border-white/10 dark:bg-white/5 dark:text-slate-200 dark:placeholder:text-slate-600"
